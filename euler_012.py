#!/usr/bin/env python

"""
Sequence of triangle numbers is generated by adding the natural numbers.
So the 7^(th) triangle number would be 1 + 2 + 3 + 4 + 5 + 6 + 7 = 28.
The first ten terms would be:

1, 3, 6, 10, 15, 21, 28, 36, 45, 55, ...

Let us list the factors of the first seven triangle numbers:

     1: 1
     3: 1,3
     6: 1,2,3,6
    10: 1,2,5,10
    15: 1,3,5,15
    21: 1,3,7,21
    28: 1,2,4,7,14,28

We can see that 28 is the first triangle number to have over five divisors.

What is the value of the first triangle number to have over five hundred divisors?
"""

import sys
import numpy as np

def orig(divisors):
    """
    My original solution to this problem.
    """
    #primes = [int(x) for x in open("prime-list.txt").read().splitlines()]
    triangle = 0
    natural = 0
    while True:
        count = 2 # for 1 and itself
        natural  += 1
        triangle += natural
        for x in range(2,np.sqrt(triangle+1)):
            if triangle % x == 0:
                count += 2
        if count>divisors:
            return triangle

def main():
    """
    The value of the first triangle number to have over five hundred divisors.
    """
    print 'Largest triangle number to have over five hundred divisors = ',orig(500)

if __name__ == "__main__":
	sys.exit(main())

